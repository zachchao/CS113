		...
		while(cont){
			...
			// a new student object
				...
				if (input == 1){
					...
					roster.add(0, tempStudent);
				}else{
					boolean found = false;
					//The counter to iterate when finding the student
					int findCounter = 0;
					//The student to check and see if it is the student we are looking for
					Student findingStudent = roster.get(findCounter);
					while(!found){
						if(roster.get(findCounter) == null){
							System.out.println("Could not find student.");
							break;
						}
						//If the students have the same name
						if(findingStudent.compareTo(tempStudent) == 0){
							found = true;
							System.out.println(roster.remove(findCounter) + " was removed from the roster");
							break;
						}
						findCounter += 1;
					}
